---
title: "Stargate Demo <br>session MARCH-21 <br>pipeline "
author: "Marc<br>BOUVET<br>March 19, 2025"
format: 
  revealjs:
    footer: "[github.com/meghall06/neair](https://github.com/meghall06/neair)&nbsp;&nbsp;&nbsp;"
    theme: [moon, custom_copy.scss]
    preview-links: auto
    chalkboard: 
      boardmarker-width: 5
from: markdown+emoji
...
(by someone else *or* by yourself in six months)
...
much more useful with a conditional such as `ifelse()`, which has three
arguments:
...
::: fragment
can access help files by typing `??tidyverse` or `??mutate` in the
console
:::
...
[**highly**]{style="color: #cc0000"} recommend using *projects* to stay
organized
...
more guidance:
[here](https://support.rstudio.com/hc/en-us/articles/200526207-Using-RStudio-Projects)
and
[here](https://r4ds.had.co.nz/workflow-projects.html#rstudio-projects)
...
click big green Code button and select "Download ZIP", then open
`neair.Rproj`
...
::: fragment
view the data with `View(faculty)` or by clicking on the data name in
...
cheat sheet of functions is
[here](https://github.com/rstudio/cheatsheets/blob/main/strings.pdf)
...
existing `faculty` data has one row per faculty, some with multiple
departments (sometimes known as *wide* data)
...
R has many useful functions for handling [relational
data]{style="color: #cc0000"}

<br>

all you need is at least one [key variable]{style="color: #cc0000"} that
connects data sets

<br>

`left_join` is most common, but there are
[more](https://r4ds.had.co.nz/relational-data.html)
...
what's the average [UG enrollment per year, per
faculty]{style="color: #cc0000"} rank?
...
`filter` to undergraduate courses only and `mutate` a new academic year
variable
...
reproduce this analysis with new data in six months**
:::
...
explainer](https://emilyriederer.netlify.app/post/team-of-packages/)

a [talk I gave](https://meghan.rbind.io/talk/boston-user/) earlier this
year on the topic
...
[STAT 545](https://stat545.com/): an online book on reproducible data
analysis in R
